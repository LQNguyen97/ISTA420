...
--the order.
select o.orderid, od.productid, od.unitprice, od.quantity, (od.unitprice*od.quantity) as LineTotal, (od.unitprice * od.quantity) / (sum(od.unitprice * od.quantity)) * 100 as PctOfTotalOrder from order_details od join orders o on od.orderid = o.orderid group by od.orderid order by od.orderid;
...
--and
select distinct c.customerid, e.employeeid, e.firstname, e.lastname, c.city, c.region, c.postalcode from customers c left join employees e on c.region = e.region order by c.customerid;
